...
public class RegelUtilsTest
{
   ...
   private ProduceratResultat resultat(UUID id, Yrkandestatus yrkandestatus)
   {
   ...
   private Yrkande yrkande(List<ProduceratResultat> resultat, Yrkandestatus yrkandestatus)
   {
   ...
   void merge_shouldIncludeAllUpdatedResults()
   {
      ...
      var updated = List.of(resultat(id1, Yrkandestatus.PLANERAT), resultat(id2, Yrkandestatus.PLANERAT));
   ...
   void merge_shouldKeepPreviousIfNotOverridden()
   {
      ...
      var updated = List.of(resultat(id1, Yrkandestatus.PLANERAT));
      var previous = List.of(resultat(id2, Yrkandestatus.YRKAT));
   ...
   void merge_shouldPreferUpdatedOverPreviousWithSameId()
   {
      ...
      var updated = List.of(resultat(id, Yrkandestatus.PLANERAT));
      var previous = List.of(resultat(id, Yrkandestatus.YRKAT));

      var result = RegelUtils.mergeProduceradeResultat(updated, previous);

      assertEquals(1, result.size());
      assertEquals(id, result.get(0).id());
      assertEquals(Yrkandestatus.PLANERAT, result.get(0).yrkandeStatus());
   }
   ...
   void createYrkande_shouldMergeResultsIntoNewInstance()
   {
      ...
      var originalResults = List.of(resultat(id2, Yrkandestatus.YRKAT));
      var updatedResults = List.of(resultat(id1, Yrkandestatus.PLANERAT));

      Yrkande original = yrkande(originalResults, Yrkandestatus.YRKAT);
